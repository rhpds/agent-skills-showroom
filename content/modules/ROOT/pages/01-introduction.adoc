...
The most reliable way to use a skill is the explicit trigger — a slash command using the skill's name. Since `brand-guidelines` is part of the `example-skills` plugin, invoke it with the namespaced command:
...
Skills can also activate automatically when Claude's assessment of your prompt matches the skill's description. This is called _implicit triggering_ — and it's less predictable than explicit triggers.

Try a prompt that closely matches the `brand-guidelines` description:
...
Because the skill's description mentions "Anthropic's official brand colors" and "style guidelines," Claude is more likely to activate it here. But implicit triggering depends on how well your phrasing matches the skill's description. A vague prompt like "review these colors" may not trigger it at all.

This is an important lesson: *write skill descriptions that are specific enough to trigger on the right prompts, but not so broad they trigger on everything.* You'll practice this when building your own skills in Module 2.
